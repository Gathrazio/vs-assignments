const validateDependencies = (deps) => {
    ...
    const recursiveDependencyCheck = (primaryDependency, dependentComponent) => { // the main body of this function checks if dependentComponent is a dependency of primaryDependency; when called recursively, the nested functions eventually determines whether primaryDependency depends on dependentComponent or anything that considers dependentComponent a dependency -- it will return false if so and true if not so.
